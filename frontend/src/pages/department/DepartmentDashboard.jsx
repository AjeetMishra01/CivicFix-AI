...
import { CheckCircle2, Clock3, FileText, MessageSquare, PlusCircle } from 'lucide-react';
...
const DepartmentDashboard = () => {
  ...
  const handleStatus = async (complaintId, status) => {
    await API.put(`/complaints/${complaintId}/status`, { status, note: remark || 'Updated' });
    setRemark('');
    fetchData();
  };
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-6">
          <div className={`rounded-2xl p-4 shadow-sm border ${statusFilter === 'all' ? 'border-brand-600 bg-brand-50' : 'border-slate-200 bg-white'} cursor-pointer`} onClick={() => setStatusFilter('all')}>
          ...
          <div className={`rounded-2xl p-4 shadow-sm border ${statusFilter === 'pending' ? 'border-brand-600 bg-brand-50' : 'border-slate-200 bg-white'} cursor-pointer`} onClick={() => setStatusFilter('pending')}>
          ...
          <div className={`rounded-2xl p-4 shadow-sm border ${statusFilter === 'in-progress' ? 'border-brand-600 bg-brand-50' : 'border-slate-200 bg-white'} cursor-pointer`} onClick={() => setStatusFilter('in-progress')}>
          ...
          <div className={`rounded-2xl p-4 shadow-sm border ${statusFilter === 'resolved' ? 'border-brand-600 bg-brand-50' : 'border-slate-200 bg-white'} cursor-pointer`} onClick={() => setStatusFilter('resolved')}>
        ...
        <div className="space-y-4">
          {filteredComplaints.map((complaint) => (
            <div key={complaint._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm cursor-pointer" onClick={() => setSelectedComplaint(complaint)}>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button className="rounded-xl bg-brand-600 px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(complaint._id, 'accepted'); }}>Accept</button>
                <button className="rounded-xl bg-civic-amber px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(complaint._id, 'in-progress'); }}>In Progress</button>
                <button className="rounded-xl bg-civic-teal px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(complaint._id, 'resolved'); }}>Resolve</button>
                <button className="rounded-xl border px-3 py-2 text-sm" onClick={(e) => { e.stopPropagation(); setSelectedComplaint(complaint); }}>
                  Remarks
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button className="rounded-xl bg-brand-600 px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(selectedComplaint._id, 'accepted'); setSelectedComplaint(null); }}>Accept</button>
              <button className="rounded-xl bg-civic-amber px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(selectedComplaint._id, 'in-progress'); setSelectedComplaint(null); }}>In Progress</button>
              <button className="rounded-xl bg-civic-teal px-3 py-2 text-sm text-white" onClick={(e) => { e.stopPropagation(); handleStatus(selectedComplaint._id, 'resolved'); setSelectedComplaint(null); }}>Resolve</button>
              <button className="rounded-xl border px-3 py-2 text-sm" onClick={(e) => { e.stopPropagation(); setShowRemarkModal(true); }}>Remarks</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-xl">
            ...
            <div className="mt-4 flex justify-end gap-3">
              ...
              <button className="px-4 py-2 rounded-xl bg-brand-600 text-white" onClick={() => { handleStatus(selectedComplaint._id, selectedComplaint.status); setShowRemarkModal(false); setSelectedComplaint(null); }}>
                Save
              </button>
            </div>
          </div>
